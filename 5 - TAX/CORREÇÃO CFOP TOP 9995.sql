UPDATE TGFITE I
 ...
 WHERE EXISTS (
   ...
   ------- SCRIPT COM MELHOR DESEMPENHO

   MERGE INTO TGFITE I
USING (
    SELECT
        C.NUNOTA,
        XT.NITEM AS SEQUENCIA,
        XT.CFOP_TAG AS CODCFO_XML
    FROM TGFCAB C
    INNER JOIN TGFNFE X 
        ON X.CHAVENFE = C.CHAVENFE
    CROSS JOIN XMLTABLE(
        XMLNAMESPACES(DEFAULT 'http://www.portalfiscal.inf.br/nfe'),
        '(/nfeProc/NFe/infNFe/det | /NFe/infNFe/det)'
        PASSING XMLTYPE(X.XML)
        COLUMNS 
            ...
            CFOP_TAG  NUMBER PATH 'prod/CFOP'
    ) XT
    WHERE C.CODTIPOPER IN (9995)
      AND XT.CFOP_TAG IS NOT NULL
) XML_CFOP
ON (
    I.NUNOTA = XML_CFOP.NUNOTA
    AND I.SEQUENCIA = XML_CFOP.SEQUENCIA
)
WHEN MATCHED THEN
    UPDATE SET I.CODCFO = XML_CFOP.CODCFO_XML
    WHERE NVL(I.CODCFO, -1) <> NVL(XML_CFOP.CODCFO_XML, -1);